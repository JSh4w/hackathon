...
async def get_service_details_by_rid(rid: str, credentials: HSPCredentials) -> Optional[Dict[str, Any]]:
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        try:
            response = await client.post(DETAILS, headers=headers, json={"rid": rid})
...
async def analyze_journey(request: ServiceMetricsRequest):
    ...
    try:
            ...
            if service_data and "serviceAttributesDetails" in service_data:
                ...
                if locations:
                    ...
                    if dep_delay is not None:
                        all_departure_delays.append(dep_delay)
                        if dep_delay > 30:
                    ...
                    if arr_delay is not None:
                        all_arrival_delays.append(arr_delay)
                        if arr_delay > 30:
        ...
        logger.info(f"📊 Departure delays: {len(all_departure_delays)} journeys")
        logger.info(f"📊 Arrival delays: {len(all_arrival_delays)} journeys")
        logger.info(f"⚠️  Extreme delays (>30min): Dep={extreme_departure_delays}, Arr={extreme_arrival_delays}")
        
        # Create histogram bins (0 to +30 minutes)
        ...
        result = {
            ...
            "departure_delays": {
                ...
                "on_time_count": sum(1 for d in all_departure_delays if d <= 0),
            ...
            "arrival_delays": {
                ...
                "on_time_count": sum(1 for d in all_arrival_delays if d <= 0),
        ...
        logger.info(f"📊 Average Departure Delay: {result['departure_delays']['avg_delay']:.1f} minutes")
        logger.info(f"📊 Average Arrival Delay: {result['arrival_delays']['avg_delay']:.1f} minutes")
        logger.info(f"✅ On-time Departures: {result['departure_delays']['on_time_count']}/{processed_count}")
        logger.info(f"✅ On-time Arrivals: {result['arrival_delays']['on_time_count']}/{processed_count}")
        logger.info("="*60)
